...
SELECT compress_chunk(c.schema_name|| '.' || c.table_name) FROM _timescaledb_catalog.chunk c, _timescaledb_catalog.hypertable ht where c.hypertable_id = ht.id and ht.table_name = 'sensor_data_compressed' and c.compressed_chunk_id IS NULL ORDER BY c.id LIMIT 3;
...
SELECT compress_chunk(c.schema_name|| '.' || c.table_name) FROM _timescaledb_catalog.chunk c, _timescaledb_catalog.hypertable ht where c.hypertable_id = ht.id and ht.table_name = 'sensor_data_compressed' and c.compressed_chunk_id IS NULL ORDER BY c.id;

SELECT * FROM sensor_data_compressed ORDER BY time DESC LIMIT 5;

-- Only the first chunks should be decompressed
:PREFIX
SELECT * FROM sensor_data_compressed ORDER BY time DESC LIMIT 5;
